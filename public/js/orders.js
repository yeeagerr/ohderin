    ...
    const html = `
        ...
        <div class="p-3 lg:p-4 border-t border-gray-200 space-y-2">
            ${
                ...
                <button onclick="deleteDraftOrder()" class="w-full px-3 lg:px-4 py-2 lg:py-2.5 border border-red-200 text-red-600 rounded-xl text-xs lg:text-sm font-medium hover:bg-red-50 transition flex items-center justify-center">
                    <svg class="w-3.5 h-3.5 lg:w-4 lg:h-4 mr-1.5 lg:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function resumeOrder() {
    ...
    // Format items untuk cart (include modifiers)
...
function deleteDraftOrder() {
    if (!currentOrderDetail || currentOrderDetail.status !== "draft") {
        alert("Hanya draft orders yang bisa dihapus");
        return;
    }

    if (
        !confirm(
            `Hapus draft order #${currentOrderDetail.order_number}? Data akan hilang dan tidak bisa dikembalikan.`,
        )
    ) {
        return;
    }

    fetch(`/kasir/pos/drafts/${currentOrderDetail.id}`, {
        method: "DELETE",
        headers: {
        ...
        .then((data) => {
            if (data.success) {
                ...
                closeOrderDetail();
                loadOrders(true); // Reload orders list
            } else {
                alert(data.message || "Gagal menghapus draft");
            }
        })
        .catch(() => {
