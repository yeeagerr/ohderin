...
@section('title', 'Stock Opname - Kasir')

@section('content')
<main class="flex-1 overflow-y-auto bg-gray-50 p-4 sm:p-6" id="mainKasirContent">
        ...
        <div class="p-4">
            <form method="GET" class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    @if(request()->hasAny(['shift', 'start_date', 'end_date']))
                        <a href="{{ route('kasir.stock-opnames.index') }}" class="px-4 py-2.5 rounded-xl bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm font-medium transition">
                            Reset
                        </a>
                    @endif
                </div>
...
<div id="addModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl max-h-[90vh] overflow-hidden">
        <div class="bg-white rounded-2xl shadow-2xl mx-4 flex flex-col max-h-[90vh]">
            ...
            <!-- Body - Scrollable -->
            <form action="{{ route('kasir.stock-opnames.store') }}" method="POST" class="flex flex-col flex-1 overflow-hidden">
                @csrf
                <div class="p-6 overflow-y-auto flex-1">
                    <div class="space-y-6">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Catatan Tambahan (Bila ada)</label>
                            <textarea name="notes" rows="2" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition" placeholder="Misal: Daging kurang bagus..."></textarea>
                        </div>
...
<script>
    ...
    async function openViewModal(id) {
        try {
            const response = await fetch(`/dashboard/stock-opnames/${id}`); // Assuming dashboard endpoint is accessible
            const data = await response.json();
            const opname = data.opname;
            const systemStocks = data.system_stocks;
            ...
            const itemsHtml = opname.items.map(item => {
                ...
                return `
                    <div class="grid grid-cols-12 gap-2 px-4 py-3 text-sm items-center hover:bg-gray-50">
                        <div class="col-span-4 font-medium text-gray-700">${item.raw_material?.name || 'Deleted Material'}</div>
                        <div class="col-span-2 text-center text-gray-500">${item.raw_material?.unit || '-'}</div>
                        <div class="col-span-2 text-right text-gray-600">${parseFloat(systemQty).toFixed(2)}</div>
                        <div class="col-span-2 text-right font-medium text-gray-800">${parseFloat(item.qty).toFixed(2)}</div>
                        <div class="col-span-2 text-right font-semibold ${diffClass}">${diffPrefix}${diff.toFixed(2)}</div>
                    </div>
                `;
            }).join('');
