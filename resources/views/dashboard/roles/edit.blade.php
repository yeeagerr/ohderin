...
<div class="flex-1 overflow-auto bg-gray-50/50">
    <div class="p-6">
        <div class="mb-6 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Role</h1>
                <p class="text-sm text-gray-500 mt-1">Perbarui nama atau hak akses menu untuk role ini.</p>
            </div>
        </div>
                ...
                <form action="{{ route('roles.update', $role) }}" method="POST">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $role->name) }}" {{ $role->name === 'Super Admin' ? 'readonly' : 'required' }} class="w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-2.5 text-sm focus:border-orange-500 focus:ring-orange-500 transition-colors {{ $role->name === 'Super Admin' ? 'opacity-60 cursor-not-allowed' : '' }}">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-3">Hak Akses Menu</label>
                            @if($role->name === 'Super Admin')
                                <div class="bg-purple-50 text-purple-700 p-4 rounded-xl border border-purple-100 mb-4 text-sm font-medium">
                                    Role Super Admin memiliki semua hak akses secara default. Pemilihan di bawah ini tidak akan memengaruhi aksesnya.
                                </div>
                            @endif
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                ...
                                <div class="flex items-start bg-gray-50 p-3 rounded-xl border border-gray-100">
                                    <div class="flex h-5 items-center">
                                        <input id="perm_{{ $key }}" name="permissions[]" value="{{ $key }}" type="checkbox" 
